...
class MyTime(hoursValue:Int, minutesValue:Int, secondsValue:Int) {
    init {
        require(hoursValue in 0..23){ "hour must be positive and less then 24" }
        require(minutesValue in 0..59){ "minute must be positive and less then 60" }
        require(secondsValue in 0..59){ "second must be positive and less then 60" }
    }
